...
class FeatureExtractor(object):
    ...
    def fit(self, X_df, y_array):
        ...
        # LEGAL ID IS THE MOST IMPORTENT FEATURE ON DECISION TREE BUT MAYBE IT OVERFEATS
        # REMOVE IT ???
        ...
        year_col = ['Year', 'Name']
        MEstimate_col = ['Activity_code (APE)']

        preprocessor = ColumnTransformer(transformers=[
                       ('zipcode', make_pipeline(zipcode_transformer, SimpleImputer(strategy='median')), zipcode_col),
                       ('num', numeric_transformer, num_cols),
                       ('head',make_pipeline(head_transformer, SimpleImputer(strategy='constant', fill_value=0)), head_cols),
